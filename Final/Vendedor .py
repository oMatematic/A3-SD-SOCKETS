            ...
            while conectado:
                try:
                    ...
                    if PORT == inicio and first:
                        ...
                        first = False
                        ProxNo = enviar_mensagem(
                            (HOST, PORT+1), indicacao, qtd, inicio, 1, PORT)
                    ...
                    if ProxNo == endereco_destino[1]:
                        if not ping():
                            ...
                            print(
                                "[AVISO] Eleição cancelada Servidor Principal no Ar")
                            conectado = False
                            servidor.close()
                            break
                    else:
                    ...
                    try:
                        if int(msg) < indicacao:
                            if ProxNo == "":
                                ...
                                print("meu numero é maior")
                                enviar_mensagem(
                                    (HOST, int(ProxNo)), indicacao, qtd, inicio, 2, PORT)
                        elif int(msg) == indicacao:
                            if not ping():
                                enviar_aviso(endereco_destino,
                                             qtd, ProxNo, PORT)
                                ip, port = endereco_destino
                                SetarIP('secondary', ip, str(port), 'ativo')
                                conectado = False
                            ...
                            if ProxNo == "":
                                ...
                                ProxNo = enviar_mensagem(
                                    (HOST, inicio+1), msg, qtd, inicio, 1, PORT)
                            
                            else:
                                print("meu numero é menor")
                                enviar_mensagem(
                                    (HOST, ProxNo), msg, qtd, inicio, 2, PORT)
                    except:
                        conn.close()
                    ...
                    print(
                        "Tempo limite de 30 segundos atingido. Nenhuma conexão foi aceita.")
                    conectado = False
            ...
            while contador < 3:
                try:
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                        print(
                            f"[Conectando] Tentando Ancoragem na porta {no_destino[1]}")
                    ...
                    print(
                        f"[ERRO] Porta {no_destino[1]} sem resposta, tentando Novamente")

                    contador += 1

            if PORT == (PortIinicial+(pcs-1)):
                print(
                    f'[SEM CONEXÃO] Porta  {no_destino[1]} sem Retorno, tentando em outra porta')
                PORT = no_destino[1]
                PORT -= (pcs-1)
                no_destino = no_destino[0], PORT
                contador = 0
            ...
            while mensagem != "fim":
                ...
                if primeira:
                    ...
                    if resposta.decode("utf-8")[-3:] == "" or resposta.decode("utf-8")[-3:] == "fim":
                        print("... >>> O servidor me respondeu:",
                              resposta.decode("utf-8")[:-3])
                        time.sleep(3)
                        mensagem = "fim"
